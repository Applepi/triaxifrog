...
if len(sys.argv) == 4:
    ...
    if command in ['mesh']:
        ...
        print(np.shape(f[filename1][::]))
        print(np.shape(corodata[::]))
        print(np.shape(corowl[::]))
        #print(f[filename1])
...
#        ret = pypret.retrieval.retriever.BaseRetriever._retrieve_begin(mesh, pulse.specturm ,weights=None)
    ...
    if command in ['import']:
        ...
        with h5py.File('ifrog.hdf5', 'w') as f:
            dset = f.create_dataset(filename1, data=testdata[:,:-1])
            dset = f.create_dataset(filename2, data=posdata)
            dset = f.create_dataset(filename2+'coro', data=posdata)
            dset = f.create_dataset('corowl', data=np.zeros((1021)))
